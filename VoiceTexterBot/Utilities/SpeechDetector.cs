...
namespace VoiceTexterBot.Utilities {
    static class SpeechDetector {
        public static string DetectSpeech (string audioPath, float inputBitrate, string languageCode) {
            Vosk.Vosk.SetLogLevel(1);
            var modelPath = Path.Combine(DirectoryExstension.GetSolutionRoot(), "Speech-models", $"vosk-model-small-{languageCode.ToLower()}");
            ...
            using(Stream source = File.OpenRead(audioPath)) {
                ...
                while(( bytesRead = source.Read(buffer, 0, buffer.Length) ) > 0) {
                    ...
                    if(rec.AcceptWaveform(buffer, bytesRead)) {
                        ...
                        textBuffer.Append(StringExstension.UppercaseFirst(sentence) + ". ");
                    }
                }
            }
